...
The default status view is now a readable summary of thread display state, a severity-ordered diagnosis block for actionable problems with duplicate portal failures collapsed into a single headline and concrete next steps, local portal health, hosted route health, managed child-process state, and relevant log file paths.
