...
The default status view is now a readable summary of thread display state, a severity-ordered diagnosis block for actionable problems with duplicate portal failures collapsed into a single headline and concrete next steps, and compact lower sections that avoid repeating the same warning text verbatim.
